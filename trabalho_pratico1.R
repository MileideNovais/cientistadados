...
va_normal_padrao <- scale(dados$Preco)
hist(va_normal_padrao)

qqnorm(dados$Preco)
qqline(dados$Preco)

shapiro.test(dados$Preco) # A hipótese nula do teste de Shapiro-Wilk é que a população possui distribuição normal.
#Portanto, um valor de p < 0.05
#indica que você rejeitou a hipótese nula, ou seja, seus dados não possuem distribuição normal.

# então p = 0.66 indica que os dados possuem uma distribuição normal.
...
#p-value é maior que alfa (10%).não temos evidência para rejeitar a hipótese nula.
...
#pelo gráfico correlação negativa entre x e y ( maior o preço menor quilometragem)
# pelo valor do coeficiente -0,82 (correlação negativa alta)- consultar tabela
...
#Podemos interpretar que no modelo ajustado que a quilometragem consegue explicar 67,76% do
#preço.
...
#A interpretação fica: A quilometragem praticados varia em 
#média 27,74% em torno da quilometragem média.
