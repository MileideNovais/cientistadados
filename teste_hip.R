# Capitulo 04 - Prof. Máiron Chaves ####
#########################################################
##
...
#Obtem o quantil (t crítico) para uma distribuição t com gl graus de liberdade. A um alfa de 
#5%
...
abline(v = -quantil,col = 'blue',lwd = 2)
abline(v = t, col = 'red')# Observe como o tcalculado é muito menor que o tcrítico. Está na 
#região de rejeição
#Obtendo o valor p
...
#Altere o valor de n de 5 em 5 observe que a medida que os graus de liberdade aumenta a 
#distribuição se aproxima da normal. Como os valores são gerados aleatoriamente 
#comportamento simétrico tende a estabilizar.
...
tabela
barplot(tabela)
#O valor críticico para uma distribuição qui-quadrado com (linhas-1)*(colunas-1)=1 grau de 
...
#Visualiza o conjunto de dados
View(dados_anova)
#Podemos utilizar os recursos de visualização da biblioteca ggplot2 para visualizar a 
#distribuição dos gastos nas populações
install.packages("ggplot2")
library(ggplot2)
ggplot(data = dados_anova, aes(x = Gastos, fill = Estado_Civil)) + 
  geom_density(alpha=0.4)+
  xlim(-50,300)
#É bastante comum também analisarmos a variabilidade nas distintas populações com uso 
#de boxplot
...
summary(anova)
# O valor p é praticamente zero. Mesmo que nosso nível de confiança fosse 99,9% ainda 
