...
#(size) de tentativas dada a probabilidade (prob) de sucesso. 
# É útil para realizar experimentos. Podemos simular qual a frequencia esperada de vendas 
#a cada dez clientes ?
...
hist(va_binomial) 
# A maior barra no histograma representa a quantidade esperada de 
#vendas
...
#########################################
# Exemplo: Uma loja recebe em média, 6 (𝝺) clientes por minuto. Qual a probabilidade de 
#que 5(x) clientes
#entrem em um minuto? 
dpois(x= 5,lambda = 6)
#Onde:
# x é a quantidade a ser testada
# lambda é a taxa média de ocorrêcia do evento em um determinado período de intervalo de 
#tempo ou espaço 
# Podemos utilizar a mesma funcao para obter a probabilidade de entrar um cliente, dois 
#clientes... quinze clientes 

va_poison <- dpois(x = 1:15, lambda = 6) 
plot(va_poison)
# Observe que os valores se distribuiem simetricamente en tormo de seis, use acontece 
#porque o paramentro
#lambda é a média (e também o desvio padrão) da distribuição de Poisson
# Também podemos obter a probabilidade acumulada de até 5 clientes entrarem na loja em 
#um minuto
...
#salário entre 
#2.400 e 2.600 ?
# Precisamos achar a probabilidade do indivíduo ter um salário de até 2.600 e subtrair pela 
#probabilidade do
...
hist(va_f)
#Vá aumentando os graus de liberdade e observe como a distribuição se aproxima da 
#normal
